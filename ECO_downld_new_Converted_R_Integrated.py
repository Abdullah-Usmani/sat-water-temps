...
os.makedirs(os.path.join(base_dir, "polygon"), exist_ok=True)
...
def classify_wetted_area(input_tiff, output_dir, crs):
    ...
    try:
        subprocess.run([
            "Rscript", R_path, input_tiff, output_dir, ref_crs, roi_path
            ], check=True)
...
def process_rasters(aid_number, date, selected_files):
    ...
    if not df["wt"].isin([1]).any():
        ...
        print(f"Water mask unreliable for {name}_{location}_{date}, using GAM4Water classification...")
        crs = LST.crs
        wetted_array = classify_wetted_area(filter_tif_path, dest_folder_filtered, crs)

        if wetted_array is None:
            print(f"GAM4Water classification failed for {date}, skipping.")
            water_mask_flag = False
            return

        # Use GAM4Water output as new water mask
        df["wt"] = wetted_array.flatten()
    else:
        water_mask_flag = True
