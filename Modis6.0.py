...
raw_path = r"/Users/ssj/Desktop/SatelliteRetrievalProject_test_1/MODISraw/"
filtered_path = r"/Users/ssj/Desktop/SatelliteRetrievalProject_test_1/MODIS/"
roi_path = r"/Users/ssj/Desktop/SateliteRetrievalProjec/polygon/test/site_full_ext_Test.shp"
log_path = r"/Users/ssj/Desktop/SatelliteRetrievalProject_test_1/logs/"
...
def process_rasters(aid_number, date, selected_files):
    ...
    def calculate_water_quality(df):
        """
        Compute water quality indices: NDWI, Turbidity Index, and Chlorophyll-a Index.
        
        Parameters:
        df (pd.DataFrame): DataFrame containing reflectance values for relevant bands.
        
        Returns:
        pd.DataFrame: DataFrame with added water quality index columns.
        """

        # Ensure bands are in float format for calculations
        df = df.copy()
        bands = ["sur_refl_b01", "sur_refl_b02", "sur_refl_b03", "sur_refl_b04"]
        
        # Convert the reflectance bands to float for calculation
        df[bands] = df[bands].astype(float)

        # NDWI Calculation (Normalized Difference Water Index)
        # NDWI = (Green - NIR) / (Green + NIR)
        # Green band: sur_refl_b02, NIR band: sur_refl_b04
        df["NDWI"] = (df["sur_refl_b02"] - df["sur_refl_b04"]) / (df["sur_refl_b02"] + df["sur_refl_b04"])
        
        # Turbidity Index (TI) -> High turbidity means murky water (more sediment)
        # TI = Red / Blue
        # Red band: sur_refl_b03, Blue band: sur_refl_b01
        df["Turbidity_Index"] = df["sur_refl_b03"] / df["sur_refl_b01"]
        
        # Chlorophyll-a Index -> Indicates algae presence
        # Chl-a = Green / Blue
        # Green band: sur_refl_b02, Blue band: sur_refl_b01
        df["Chlorophyll_a_Index"] = df["sur_refl_b02"] / df["sur_refl_b01"]
        
        # Replace invalid values (infinity or NaN) with NaN
        df.replace([np.inf, -np.inf], np.nan, inplace=True)

        return df

   # output_csv = os.path.join(dest_folder_filtered, f"{name}_{location}_{date}_filter_WQ.csv")
    ...
    # Remove rows where the cloud_mask is 1 (cloudy)
    df_cleaned = df[df["cloud_mask"] != 1]

    # Remove rows where the water_mask is 0 (not water)
    df_cleaned = df_cleaned[df_cleaned["water_mask"] != 0]

    # Apply water quality calculations
    df_cleaned = calculate_water_quality(df_cleaned)

    # Check the cleaned and processed DataFrame with water quality indices
    print(df_cleaned[["NDWI", "Turbidity_Index", "Chlorophyll_a_Index"]].head())
    ...
    # Save the cleaned data with the calculated indices to CSV
    df_cleaned.to_csv(filter_csv_path, index=False) 
    # Define a separate output file for water quality indices
    wq_csv_path = os.path.join(dest_folder_filtered, f"{name}_{location}_{date}_WaterQuality.csv")

    # Select only the water quality index columns to save separately
    df_wq = df_cleaned[["NDWI", "Turbidity_Index", "Chlorophyll_a_Index"]]

    # Save water quality indices separately
    df_wq.to_csv(wq_csv_path, index=False)

    print(f"Filtered data saved at: {filter_csv_path}")
    print(f"Water quality indices saved at: {wq_csv_path}")
